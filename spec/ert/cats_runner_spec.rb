...
YAML
  ...
  context 'vsphere' do
    before do
      ...
      allow(cats_runner).to receive(:system).and_return(true)
    end

    it 'directly targets the microbosh' do
      expect(bosh_command).to receive(:target).and_return('the bosh target command')
      expect(cats_runner).to receive(:system).with('the bosh target command')
    ...
    it 'sets the deployment to the downloaded manifest' do
      expect(cats_runner).to receive(:system).with("bosh_command deployment temp_dir/#{environment_name}.yml")
    ...
    context 'when running the cats errand' do
      context 'in an environment with full internet connectivity' do
        it 'runs the acceptance-tests errand' do
          expect(cats_runner).to receive(:system).with('bosh_command run errand acceptance-tests')
        ...
        context 'when the acceptance-tests errand fails' do
          it 'raises a RuntimeError' do
            expect(cats_runner).to receive(:system).with('bosh_command run errand acceptance-tests').and_return(false)
...
YAML
        ...
        it 'runs the acceptance-tests errand' do
          expect(cats_runner).to receive(:system).with('bosh_command run errand acceptance-tests-internetless')
        ...
        context 'when the acceptance-tests-internetless errand fails' do
          it 'raises a RuntimeError' do
            expect(cats_runner).to(
              receive(:system).with('bosh_command run errand acceptance-tests-internetless')
                .and_return(false)
            )
            expect { cats_runner.run_cats }.to raise_error(RuntimeError, 'CF Acceptance Tests failed')
          end
    ...
    context 'when a bosh command fails' do
      context 'when bosh target fails' do
        it 'raises a RuntimeError' do
          expect(cats_runner).to receive(:system).with('the bosh target command').and_return(false)
      ...
      context 'when bosh deployment fails' do
        it 'raises a RuntimeError' do
          expect(cats_runner).to(
            receive(:system).with("bosh_command deployment temp_dir/#{environment_name}.yml")
              .and_return(false)
          )
