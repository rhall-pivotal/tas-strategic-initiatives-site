...
resource "template_file" "ops_manager" {
  template = "${file(\"templates/iam_ops_manager_policy.json\")}"
...
resource "template_file" "ert" {
  template = "${file(\"templates/iam_ert_buckets_policy.json\")}"
  depends_on = [
...
resource "aws_iam_policy" "ops_manager" {
  ...
  policy = "${template_file.ops_manager.rendered}"
}

resource "aws_iam_policy" "ert" {
  name = "${var.env_name}_ert"
  policy = "${template_file.ert.rendered}"
}
