...
var _ = Describe("Networking", func() {
	...
	Describe("Routing", func() {
		Describe("prune_all_stale_routes", func() {
			var (
				inputProperties     map[string]interface{}
			...
			Describe("when the property is set", func() {
				BeforeEach(func() {
					inputProperties = map[string]interface{}{
						".properties.router_prune_all_stale_routes": "true",
					}
				})
				...
				It("sets the prune_all_stale_routes", func() {
					...
					pruningOn, err := job.Property("router/prune_all_stale_routes")
					Expect(err).NotTo(HaveOccurred())
					Expect(pruningOn).To(BeTrue())
				})
			})
